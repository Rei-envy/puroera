...
# アプリケーションを作成した背景
自分のような未経験エンジニアをペルソナとして、自分がプログラミング学習中にエラー解決に時間がかかることと、分からないことに対して、もっと効率的に解決できないかと思いました。<br>
課題の解決のために、誰かのエラーを解決した方法を教えてもらうことが必要だと考えました。また、解決方法が共有されていない場合は、質問をし経験者に教えてもらえるようにする必要もあると考えました。<br>
また、エラーを蓄積することでエラー解消までの時間を短縮でき、エラー内容をアウトプットすることで、スキルアップにも繋がります。<br>
また、既存のプラットフォームでは、コミュニケーションを取るためにSNSでフォローをしてDMを送るという手間がかかると感じたため、チャットルームもつけることにしました。
...
# 工夫したポイント
ユーザー目線で、シンプルかつ使いやすいようにするためにフロント実装に特に力を入れました。<br>
工夫したポイントは4つあります。
1. ヘッダー部分からどのページにも遷移できるようにしています。
2. 配色にもこだわり、メインカラーを水色とし、ベースカラーを白と黒としました。質問ページではメインからを紫として、共有ページとの違いを出しています。
3. 全体的に余白を空けブロックごとに詰まった印象にならないように注意しました。
4. ロゴも自作し、ユーザー登録や新規投稿ページでは、フリー素材のアイコン等を用いて、ぱっと見て分かるような、デザインにしました。

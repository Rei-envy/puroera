...
# アプリケーション概要
プログラミングのエラー内容を共有し、他のエンジニアの解決に貢献する。
共有された内容を見ることで、自分のエラーを解決できる。
自分が起きたエラーを質問して、回答を依頼することができる。
チャットルームで他のエンジニアとコミュニケーションが取れる。
...
## 共有内容の投稿
1. ユーザーの新規登録を行う。
2. 右上の「共有する」ボタンから、共有内容の投稿ページに遷移する。
3. 内容（タイトル、カテゴリー、状況と仮説、解決方法、感想）を記述する。※感想は任意
4. ページ下部の「投稿する」ボタンを押す。

## コメントの投稿
1. ユーザーの新規登録を行う。
2. トップページの「共有掲示板」の一覧でタイトルをクリックし、詳細ページに遷移する。
3. 詳細ページ下部のコメントフォームから、コメントができる。

## 質問内容の投稿
1. ユーザーの新規登録を行う
2. 右上の「質問する」ボタンから、質問内容の投稿ページに遷移する
3. 内容（タイトル、カテゴリー、状況と仮説、やったこと、感想）を記述する。※やったことと、感想は任意
4. ページ下部の「質問する」ボタンを押す

## 回答の投稿
1. ユーザーの新規登録を行う。
2. ヘッダーの「質問掲示板」をクリックし、質問掲示板ページに遷移する。
3. 質問掲示板の一覧でタイトルをクリックし、詳細ページに遷移する。
4. 詳細ページ下部の回答フォームから、回答ができる。

## チャットルームでの交流
1. ユーザーの新規登録を行う。
2. ヘッダーの「マイチャット」をクリックし、マイチャットページに遷移する。
3. 右上の「チャットを作成する」ボタンをクリックし、チャットルーム作成ページに遷移する。
4. チャットルーム名とチャット相手を選び、「ルームを作成する」ボタンを押す。
5. チャットルームに遷移後、ページ右下にメッセージフォームが出現するため、そこでメッセージを入力し、「送信」ボタンを押すと送信できる。

# アプリケーションを作成した背景
ペルソナを自分とし、自分がプログラミング学習中にエラー解決に時間がかかることと、分からないことに対して、辛さを感じていたことを課題とした。
課題の解決のために、誰かのエラーを解決した方法を教えてもらうことが必要だと考えた。また、解決方法が共有されていない場合は、質問がし経験者に教えてもらえるようにする必要もあると考えた。
また、既存のqiitaやteratailなどのエンジニアに質問できるプラットフォームでは、コミュニケーションを取るためにTwitterなどのSNSでフォローをしてDMを送るという手間がかかると感じたため、チャットルームもつけることにした。

# 洗い出した要件
https://docs.google.com/spreadsheets/d/1ld-nNYz02Ex-G_XjQkZQYn9lijlZ4FRtIBl0qZ07IT0/edit#gid=982722306
...
# 工夫したポイント
フロント実装に特に力を入れました。
ユーザー目線で、シンプルかつ使いやすいように不要なものは減らすようにしました。
ヘッダー部分からどこページにも遷移できるようにしています。
チャットルームでは、自分のメッセージは右から、相手のメッセージは左からにするようにし、既存のメッセージアプリであるLINEなどと近いようなデザインにし、使いやすくしました。
また、全体的に余白を空けブロックごとに詰まった印象にならないように注意しました。
配色にもこだわり、メインカラーを水色とし、ベースカラーを白と黒としました。質問ページではメインからを紫として、共有ページとの違いを出しています。
ロゴも自作し、ユーザー登録や新規投稿ページでは、フリー素材のアイコン等を用いて、文字ばかりではなくぱっと見て分かるような、デザインにしました。

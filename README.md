...
# アプリケーションを作成した背景
ペルソナを自分とし、自分がプログラミング学習中にエラー解決に時間がかかることと、分からないことに対して、辛さを感じていたことを課題とした。<br>
課題の解決のために、誰かのエラーを解決した方法を教えてもらうことが必要だと考えた。また、解決方法が共有されていない場合は、質問がし経験者に教えてもらえるようにする必要もあると考えた。<br>
...
# 工夫したポイント
フロント実装に特に力を入れました。<br>
ユーザー目線で、シンプルかつ使いやすいように不要なものは減らすようにしました。<br>
ヘッダー部分からどこページにも遷移できるようにしています。<br>
チャットルームでは、自分のメッセージは右から、相手のメッセージは左からにするようにし、既存のメッセージアプリであるLINEなどと近いようなデザインにし、使いやすくしました。<br>
また、全体的に余白を空けブロックごとに詰まった印象にならないように注意しました。
配色にもこだわり、メインカラーを水色とし、ベースカラーを白と黒としました。質問ページではメインからを紫として、共有ページとの違いを出しています。<br>
